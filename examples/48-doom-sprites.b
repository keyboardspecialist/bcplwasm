// 48-doom-sprites: 47 + sprite rendering for common THING types.
//
// What's new vs 47:
//   - col_z[W] per-frame z-buffer: stores min cy seen per column
//     during wall rendering. Sprites test against it for occlusion.
//   - Hardcoded mapping for common Doom1 THING types → sprite lump
//     name (barrels, ammo, armor, health bonus, etc.).
//   - Sprite cache (parallel to texture cache): decodes the patch
//     header for w / h / leftofs / topofs and composites RGBA into
//     wasm memory once per unique sprite.
//   - After BSP wall pass, iterate THINGS, project each visible
//     sprite into screen-space, render column-by-column via
//     drawwallcol with transparency mode.
//
// Limitations:
//   - No depth sort between sprites. THINGS-order overlap may flicker
//     where two sprites overlap on screen.
//   - No rotation frames. Uses ?A0 sprite for pickups, ?A1 for monsters.
//   - Sprite vs wall occlusion via col_z (single-depth per column).
//     For semi-transparent walls (portal mid-tex) this can be off.
//
...
{ LET v1, v2 = 0, 0
  ...
  { LET t     = ((col_x - sx1) * 1024) / dx_span
    ...
    { LET offset  = col_x - W/2
      ...
      // Ceiling band (above front ceil) — flat texture if available.
