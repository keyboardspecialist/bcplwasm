// 44-doom-wall-tex: Doom BSP renderer with real wall textures.
//
// Pipeline:
//   1. PLAYPAL  — 256-colour palette, first variant.
//   2. PNAMES   — patch-name table (each entry = 8-char lump name).
//   3. TEXTURE1 — composite texture definitions (name + size + patch list).
//   4. For every unique texture referenced by any SIDEDEF in the map,
//      composite the patches into an RGBA buffer in wasm memory.
//   5. For each wall column the renderer looks up the relevant sidedef
//      texture, computes a perspective-correct U coord, and calls
//      Sys_drawwallcol — which samples the texture per pixel and wraps
//      V modulo tex_h so tall walls tile properly.
//
// Setup: same as 43 — upload a Doom WAD via Assets, Compile & Run.
// Pre-build of textures takes ~1s on E1M1 the first time.
//
// Known limitations (for later phases):
//   - No texture pegging flags (everything anchored at the top of its
//     band); midtex on portals not yet drawn.
//   - Per-column U uses real wall length via integer sqrt — accurate
//     enough; perspective-correct interp of u/z and 1/z.
//   - Light diminishing not implemented (textures rendered at full RGB).
//   - Floor/ceiling still flat color via fill_remaining.
//
...
{ TEST cidx < 0
  ...
  ELSE { LET tw = tex_w_vec!cidx
         ...
         // NB: BCPL's `%` is byte-fetch (s_byteap), NOT modulo.
         // Use MOD / REM for arithmetic modulo.
...
{ LET v1, v2 = 0, 0
  ...
  { LET t     = ((col_x - sx1) * 1024) / dx_span
    ...
    // Ceiling band (above front ceil) — flat colour.
    ...
    // Floor band (below front floor) — flat colour.
    { LET top = col_top!col_x
      LET bot = col_bot!col_x
      IF bot > y_cc DO
      { LET y0 = y_cc + 1
