(module
  ...
  (func $fn_L18 (export "fn_L18") (type $bcpl_fn)
    ...
    (local.set $t9 (i32.add (global.get $TB) (i32.const 5))) ;; LF L16
    (i32.store (i32.add (local.get $Pb) (i32.const 28)) (local.get $t7))
    ...
    (local.set $t10 (i32.add (global.get $TB) (i32.const 5))) ;; LF L16
    (i32.store (i32.add (local.get $Pb) (i32.const 32)) (local.get $t8))
    ...
    (local.set $t11 (i32.add (global.get $TB) (i32.const 5))) ;; LF L16
    (i32.store (i32.add (local.get $Pb) (i32.const 36)) (local.get $t9))
    ...
    (if (i32.gt_s (local.get $t9) (local.get $t10)) (then
      ...
      ) ;; close $__case_6 — LAB L24
    ;; line 0:75
    (global.set $__line (i32.const 75))
    (if (i32.ne (global.get $__bp_armed) (i32.const 0))
        (then (call $__break)))
    (local.set $t9 (local.get $t7)) ;; LP promoted P!7
    (local.set $t9 (i32.sub (i32.const 0) (local.get $t9)))
    (local.set $t10 (local.get $t7)) ;; LP promoted P!7
    ...
    (if (i32.ne (global.get $__bp_armed) (i32.const 0))
        (then (call $__break)))
    (local.set $t10 (i32.add (global.get $SB) (i32.const 12))) ;; LSTR
    ...
    (local.set $t15 (i32.add (global.get $TB) (i32.const 3))) ;; LF L14
    ...
    (local.set $t16 (i32.add (global.get $TB) (i32.const 4))) ;; LF L15
    ...
    (if (i32.ne (global.get $__bp_armed) (i32.const 0))
        (then (call $__break)))
    (local.set $t7 (i32.add (global.get $SB) (i32.const 19))) ;; LSTR
    ...
    (local.set $t12 (i32.add (global.get $TB) (i32.const 5))) ;; LF L16
    (i32.store (i32.add (local.get $Pb) (i32.const 40)) (local.get $t10))
    (i32.store (i32.add (local.get $Pb) (i32.const 44)) (local.get $t11))
    (i32.store (i32.add (local.get $Pb) (i32.const 28)) (global.get $P)) ;; save P
    (i32.store (i32.add (local.get $Pb) (i32.const 32)) (i32.const 0)) ;; return addr placeholder
    ...
    (if (i32.eq  (local.get $t6) (local.get $t7)) (then
      ...
      ) ;; close $__case_10 — LAB L28
    (local.set $t9 (i32.const 0))
    (local.set $t10 (local.get $t5)) ;; LP promoted P!5
    (local.set $t9 (i32.add (local.get $t9) (local.get $t10)))
    (local.set $t9 (i32.load (i32.shl (local.get $t9) (i32.const 2))))
    (local.set $t10 (i32.load (i32.add (i32.shl (global.get $G) (i32.const 2)) (i32.const 344))))
    (i32.store (i32.add (local.get $Pb) (i32.const 36)) (local.get $t9))
    (i32.store (i32.add (local.get $Pb) (i32.const 24)) (global.get $P)) ;; save P
    (i32.store (i32.add (local.get $Pb) (i32.const 28)) (i32.const 0)) ;; return addr placeholder
    ...
    (local.set $t13 (i32.add (global.get $TB) (i32.const 3))) ;; LF L14
    (i32.store (i32.add (local.get $Pb) (i32.const 48)) (local.get $t12))
    ...
    (local.set $t14 (i32.add (global.get $TB) (i32.const 4))) ;; LF L15
    (i32.store (i32.add (local.get $Pb) (i32.const 52)) (local.get $t13))
    ...
    (drop (call_indirect $ftable (type $bcpl_fn) (local.get $t10)))
    (local.set $Pb (i32.shl (global.get $P) (i32.const 2)))
      (local.set $__lab (i32.const 13)) (br $__dispatch) ;; JUMP L32
      ) ;; close $__case_12 — LAB L30
    ;; line 0:111
    (global.set $__line (i32.const 111))
    (if (i32.ne (global.get $__bp_armed) (i32.const 0))
        (then (call $__break)))
    (local.set $t8 (i32.add (global.get $SB) (i32.const 58))) ;; LSTR
    (local.set $t9 (i32.load (i32.add (i32.shl (global.get $G) (i32.const 2)) (i32.const 376))))
    (i32.store (i32.add (local.get $Pb) (i32.const 32)) (local.get $t8))
    (i32.store (i32.add (local.get $Pb) (i32.const 20)) (global.get $P)) ;; save P
    (i32.store (i32.add (local.get $Pb) (i32.const 24)) (i32.const 0)) ;; return addr placeholder
    ...
    (local.set $Pb (i32.shl (global.get $P) (i32.const 2)))
      ...
      ) ;; close $__case_13 — LAB L32
    ;; line 0:113
    (global.set $__line (i32.const 113))
    (if (i32.ne (global.get $__bp_armed) (i32.const 0))
        (then (call $__break)))
    (local.set $t7 (local.get $t3)) ;; LP promoted P!3
    (local.set $t8 (i32.add (global.get $TB) (i32.const 1))) ;; LF L12
    (i32.store (i32.add (local.get $Pb) (i32.const 28)) (local.get $t7))
    (i32.store (i32.add (local.get $Pb) (i32.const 16)) (global.get $P)) ;; save P
    (i32.store (i32.add (local.get $Pb) (i32.const 20)) (i32.const 0)) ;; return addr placeholder
    (i32.store (i32.add (local.get $Pb) (i32.const 24)) (local.get $t8)) ;; entry fn_idx
    ...
    (if (i32.ne (global.get $__bp_armed) (i32.const 0))
        (then (call $__break)))
    (local.set $t7 (i32.add (global.get $SB) (i32.const 68))) ;; LSTR
    (local.set $t8 (i32.load (i32.add (i32.shl (global.get $G) (i32.const 2)) (i32.const 356))))
    (i32.store (i32.add (local.get $Pb) (i32.const 28)) (local.get $t7))
    (i32.store (i32.add (local.get $Pb) (i32.const 16)) (global.get $P)) ;; save P
    (i32.store (i32.add (local.get $Pb) (i32.const 20)) (i32.const 0)) ;; return addr placeholder
  ...
  (elem $ftab funcref (ref.func $fn_L10) (ref.func $fn_L12) (ref.func $fn_L13) (ref.func $fn_L14) (ref.func $fn_L15) (ref.func $fn_L16) (ref.func $fn_L18))

  ;; static data — passive segment (72 words)
  (data $stat "\0A\61\72\65\6E\61\20\4F\4F\4D\0A\00\21\76\31\21\35\20\3D\20\25\6E\20\20\76\32\21\31\30\20\3D\20\25\6E\20\20\76\33\21\33\20\3D\20\25\6E\0A\00\00\1B\61\72\65\6E\61\20\75\73\65\64\3A\20\25\6E\20\2F\20\25\6E\20\77\6F\72\64\73\0A\0A\18\61\66\74\65\72\20\72\65\73\65\74\3A\20\75\73\65\64\20\3D\20\25\6E\0A\0A\00\00\00\15\73\71\75\61\72\65\73\20\6C\69\73\74\20\28\4C\49\46\4F\29\3A\20\00\00\35\61\72\65\6E\61\20\75\73\65\64\3A\20\25\6E\20\2F\20\25\6E\20\77\6F\72\64\73\20\20\20\6E\6F\64\65\73\3A\20\36\20\78\20\32\20\3D\20\31\32\20\77\6F\72\64\73\0A\0A\00\00\2D\6F\76\65\72\73\69\7A\65\20\61\6C\6C\6F\63\20\63\6F\72\72\65\63\74\6C\79\20\72\65\66\75\73\65\64\20\28\75\73\65\64\20\3D\20\25\6E\29\0A\00\00\27\75\6E\65\78\70\65\63\74\65\64\3A\20\6F\76\65\72\73\69\7A\65\20\68\61\6E\64\6F\75\74\20\73\75\63\63\65\65\64\65\64\0A\0C\61\72\65\6E\61\20\66\72\65\65\64\0A\00\00\00")
