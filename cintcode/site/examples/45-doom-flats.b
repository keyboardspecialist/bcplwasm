// 45-doom-flats: 44 + textured floor and ceiling flats.
//
// What's new vs 44:
//   - Decode 64x64 flats (raw palette-indexed bytes) referenced by
//     SECTOR.floor_tex / ceil_tex into RGBA buffers in wasm memory.
//   - Per-column ceiling/floor bands now call Sys_drawflatspan, which
//     does the per-pixel reverse projection (Doom-style floor cast)
//     and samples the flat texture instead of drawing a flat colour.
//   - The "F_SKY1" sky marker is detected and falls back to sky_col
//     so we don't try to decode a 0-size lump.
//
