// (c) Copyright: Martin Richards   19 Aug 2023

/*
Change log

19/08/2023
Added estimate icountmax used to set an initial guess in the rootnode
of the rate of execution of Cintcode instructions.  Made many other
changes mainly from the new file names of the resident parts of the
system, and the new implemetation of tty input. All this was a major
change.
...
//}
  ...
  UNLESS rootnode!rtn_quietflag DO
    sawritef("*nBCPL %n-bit Cintcode System (13 Dec 2023)*n", BITSPERBCPLWORD)
  ...
  // Initialize the date and time values in the rootnode
  rootnode!rtn_days  := 0              // MR 14/08/2023
  rootnode!rtn_msecs := 0              // MR 14/08/2023
  //rootnode!rtn_usecs := -12345         // Not used MR 14/08/2023
  ...
  // Set icountmax in the rootnode to an estimate of the number of
  // Cintcode instructions executed per milli-second.
  FOR count = 1 TO 3 DO
  { LET datstamp1 = VEC 2
    LET datstamp2 = VEC 2
    LET msecsdiff = 0
    LET x = 0
    rtn_icountmax!rootnode := 70_000
    //FOR i = 1 TO 40_000_000 DO x := x + i MOD 10 & #xFF
    datstamp(datstamp1);
    FOR i = 1 TO 1_000_000 DO x := x + i MOD 1000 & #xFFFF
    datstamp(datstamp2);
    //writef("icountmax=%n*n", rtn_icountmax!rootnode)
    msecsdiff := datstamp2!1 - datstamp1!1
    //writef("msecsdiff)=%i4*n", msecsdiff)
    IF msecsdiff < 10 LOOP
    rtn_icountmax!rootnode := muldiv(92_000, 200, msecsdiff)
    UNLESS rootnode!rtn_quietflag DO
      writef("Estimated %n Cintcode instructions per milli-second*n",
              rtn_icountmax!rootnode)
    // This value can be compared with that given by sysinfo.
    BREAK
  }
    
  IF rootnode!rtn_boottrace DO
  { sawritef("Estimated Cincode intructions per second = %n*n",
             rtn_icountmax!rootnode)
  }
  
  IF rootnode!rtn_boottrace>1 DO
  { regs!r_count := 1_000_000_000 // The Cincode count register
    sys(Sys_tracing, FALSE)
    sawritef("boot: instruction tracing turned off*n")
  }
...
{ // This function runs using the KLIB or CLI stack and global
  ...
  // rootnode  -- set to point to the root node (typically at 100)
...
//abort(1001)
  ...
  { sawritef("startroot: can now use normal stream i/o*n")
    sawritef("startroot: trying to load syscin/clisys*n")
  }

  UNLESS globin(loadseg("syscin/clisys")) DO
  { sawritef("Unable to start CLISYS.*n")
    sys(Sys_quit, 0)
  }

  IF rootnode!rtn_boottrace DO
  { sawritef("startroot: loaded syscin/clisys successfully*n")
    sawritef("startroot: now entering the clisys*n")
  }
       ...
       ELSE TEST globword<=n<=globword+gptr!0
            ...
            ELSE TEST -10000000<=n<=10000000
                 ...
                 ELSE TEST n &
		           //(-100_000.0 < n < -0.00001 |
			   //  0.00001   < n < 100_000.0)
		           (sys(Sys_flt, fl_mk, -1, 5) #< n &
			    n #< sys(Sys_flt, fl_mk, -1, -5) |
			    sys(Sys_flt, fl_mk,  1, -5) #< n &
			    n #< sys(Sys_flt, fl_mk,  1, 5))
		      THEN writef("  %8.3f", n)
		      ELSE writef("   #x%x8", n)
...
AND flop2str(op) = VALOF SWITCHON op INTO
{ DEFAULT:           RESULTIS "?"
  CASE fl_avail:     RESULTIS "avail"
  CASE fl_mk:        RESULTIS "mk"
  CASE fl_unmk:      RESULTIS "unmk"
  ...
  CASE fl_acos:      RESULTIS "acos"
  CASE fl_asin:      RESULTIS "asin"
  CASE fl_atan:      RESULTIS "atan"
  CASE fl_atan2:     RESULTIS "atan2"
  CASE fl_cos:       RESULTIS "cos"
  CASE fl_sin:       RESULTIS "sin"
  CASE fl_tan:       RESULTIS "tan"
  CASE fl_cosh:      RESULTIS "cosh"
  CASE fl_sinh:      RESULTIS "sinh"
  CASE fl_tanh:      RESULTIS "tanh"
  CASE fl_exp:       RESULTIS "exp"
  CASE fl_frexp:     RESULTIS "frexp"
  CASE fl_ldexp:     RESULTIS "ldexp"
  CASE fl_log:       RESULTIS "log"
  CASE fl_log10:     RESULTIS "log10"
  CASE fl_pow:       RESULTIS "pow"
  CASE fl_sqrt:      RESULTIS "sqrt"
  CASE fl_ceil:      RESULTIS "ceil"
  CASE fl_floor:     RESULTIS "floor"
  CASE fl_modf:      RESULTIS "modf"
  CASE fl_N2F:       RESULTIS "N2F"
  CASE fl_F2N:       RESULTIS "F2N"
  CASE fl_radius2:   RESULTIS "radius2"
  CASE fl_radius3:   RESULTIS "radius3"
  CASE fl_64to32:    RESULTIS "64to32"
  CASE fl_32to64:    RESULTIS "32to64"
}
