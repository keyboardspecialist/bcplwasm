...
//sawritef("findstream: name=%s*n", name)
  IF console DO
  { // Test whether the console stream has been opened already
  ...
  IF isstderr DO
  { IF id=id_outscb & rootnode!rtn_stderr  RESULTIS rootnode!rtn_stderr
  }
  
  scb := getvec(scb_upb)
  UNLESS scb RESULTIS 0

//sawritef("findstream: task %i2 allocating scb =%i6 for %s*n",
//          taskid, scb, name)
  ...
  // Copy (possibly truncated) stream name into the scb
  { LET len = name%0
  ...
  { // Console stream being opened for the first time
    ...
    IF id=id_outscb DO
    { scb!scb_wrfn   := cnslwrfn // fn to deplete current buffer
  ...
  IF isstderr DO
  { // stderr stream being opened for the first time
    LET buf = getvec(4095/bytesperword) // Room for 4096 bytes
    UNLESS buf DO { freevec(scb); RESULTIS 0 }
    scb!scb_type    := scbt_console
    scb!scb_buf     := buf
    scb!scb_bufend  := 4096
    scb!scb_endfn := errendfn

    IF id=id_outscb DO
    { scb!scb_wrfn   := errwrfn // errwrfn // fn to deplete current buffer
      rootnode!rtn_stderr := scb
      RESULTIS scb
    }
    // Bad stream id
    freevec(scb)
...
//sawritef("DLIB: Opening stream to/from NIL:*n")
  ...
  { // Open a RAM stream.
    ...
    scb!scb_wrfn    := ramwrfn // This automatically expands the buffer
                               // if needed.
//sawritef("DLIB: Opening stream to/from RAM:*n")
  ...
  { sawritef("TCP connections not yet available in cintsys*n")
    freevec(scb)
    RESULTIS 0
  }
...
{ // Replace the buffer by one of twice the size
  ...
  LET newbufend = oldbufend*2    // Double the buffer size
...
AND cnslendfn(scb) = VALOF
{ //freevec(scb!scb_buf)
  RESULTIS TRUE
}

AND errwrfn(scb) = VALOF
{ LET buf = scb!scb_buf
//sawritef("DLIB: errwrfn(%n) called*n", scb)
//sawritef("DLIB: errwrfn: buf=%n pos=%n end=%n*n",
//          scb!scb_buf, scb!scb_pos, scb!scb_end)

  FOR i = 0 TO scb!scb_pos-1 DO sys(Sys_errwrch, buf%i)
  scb!scb_pos := 0
  scb!scb_end := 0  // No valid data
  RESULTIS TRUE
}

AND errendfn(scb) = VALOF
{ //freevec(scb!scb_buf)
  RESULTIS TRUE
}
...
//          filename, path->path,"null")
  ...
  UNLESS sys(Sys_openread, filename, path, @fp) DO  // MR 8/5/03
  {
...
//sawritef("DLIB: fh0findinput scb %n fp %n buf %n*n", scb, fp, buf)
  ...
  scb!scb_fd      := fp      // Allow for 64 bit file pointer
  scb!scb_fd1     := fp1
...
{ LET fp, fp1 = 0, 0
  ...
  UNLESS sys(sysop, filename, @fp) DO
  { result2 := 100
    RESULTIS FALSE
  }
...
//sawritef("DLIB: fh0findoutput scb %n  buf %n*n", scb, buf)
...
//                result2=101  Can't allocate buffer
{ LET fp, fp1 = 0, 0
  LET buf, res1, res2 = 0, 1, 0
  LET filesize = 0
  LET filename = VEC 50
  trfilename(name, filename)

  // open the file for input and output
  UNLESS sys(Sys_openreadwrite, filename, @fp) DO
  { result2 := 100
    RESULTIS FALSE
  }
...
//sawritef("DLIB: fh0findinoutput scb %n  fp=(%n %n) buf %n*n", scb, fp, fp1, buf)
  ...
  scb!scb_lblock  := filesize/buflen //+ 1  // MR 16/4/02 MR 29/7/02
  scb!scb_ldata   := filesize MOD buflen  // MR 16/4/02
...
//               block, lblock,  blkno, pos, end)
  ...
  // The stream must be a disc or RAM file
  UNLESS (type=scbt_file | type=scbt_ram)
  { sawritef("DLIB: point can only used with file and ram streams*n")
    abort(999)
    RESULTIS FALSE
  }
...
//sawritef("DLIB: point setting scb block=%n pos=%n*n", blkno, pos)
  ...
  // Reading a block from file or ram is only allowed if
  // reading is permitted.

  UNLESS id=id_inscb | id=id_inoutscb DO
  { sawritef("DLIB: The stream did not permit reading*n")
    abort(999)
    RESULTIS FALSE
  }
...
//sawritef("DLIB: putbuf write %n bytes at offset %n*n", end, offset)
  IF sys(Sys_write, fdptr, scb!scb_buf, end) < 0
    RESULTIS FALSE
...
//            block, offset)
  UNLESS sys(Sys_seek, fdptr, offset) RESULTIS FALSE
//sawritef("DLIB: fh0getbuf file position now %n*n", sys(Sys_tell, fdptr))
...
AND delay(msecs) = sys(Sys_delay, msecs)
