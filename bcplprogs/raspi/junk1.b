...
GLOBAL {
  ...
  FLT cgndot; FLT cgwdot; FLT cghdot // Aircraft velocity in world coordinates
                                     // Note that the velocity is not
				     // necessarily in direction t.
  // The aerodynamic forces affecting the aircraft depend on both the
  // aircraft orientation and its velocity.
  rfw := rfw - 200.0 * ldot
...
//abort(1789)
  ...
  // Calculate the new speed in (t,w,l) coordinates.
  tdot := tdot + ft/massg/steprate
  wdot := wdot + fw/massg/steprate
  ldot := ldot + fl/massg/steprate

  // Calculate new CG velocity in world coodinates.
  calccgdot()
...
AND calccgdot() BE
{ cgndot := (tdot*ctn + wdot*cwn + ldot*cln) / steprate
  cgwdot := (tdot*ctw + wdot*cww + ldot*clw) / steprate
  cghdot := (tdot*cth + wdot*cwh + ldot*clh) / steprate
}
