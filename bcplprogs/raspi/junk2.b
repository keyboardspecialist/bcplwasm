...
GLOBAL {
  ...
  FLT rtdot     // Rate of rotation about axes t, w and l.
  FLT rwdot
  FLT rldot
...
{ // Since the dimentions of the Tigermoth are given in feet and
  ...
  // (t,w,l) is the point on the aircraft where the force is applied
  // and (pt,pw,pl) is the force vector components measured in
  // poundals.

  // The centre of gravity (CG) of the aircraft is at (cgtpos,0,0).
  // The force will give it an acceleration of (pt/M,pw/M,pl/M)
  // whe M is the mass of the aircraft.
  
  // If the force is not applied to the centre of gravity there
  // will also be a rotational effect, whose size relies on the
  // following reasoning.


  // The rotational effect can consider each axis separately. For
  // instance pt affects the rotation about the w axis acting at a
  // distance l. This will contribute a rotational force of pt*l
  // resulting in a rotational acceleration of pt*l/I where I is
  // the moment of inertia of the aircraft about the the axis
  // parallel to w through the CG. pt also affects the rotation
  // about l, and pw and pl similarly affect the rotations about
  // the other axes. Note that the rate of rotation about an axis
  // through the CG is the same as the rate of rotation abot the
  // origin.

  // If the CG is fixed in space it is clear that the rotational
  // acceleration is F*r/I. But if the CG is allowed to accelerate
  // this result is not so obvious. We can confirm that the rotational
  // acceleration is still F*r/I using the method described in Lecture 3
  // of Susskind's Stanford lectures on Classical Dynamics (available
  // on the web).

  // Consider an object with mass M and moment of inertia I about one
  // axis. If a force F is applied in a direction orthogonal to the
  // axis as a distance r. The object can be replaced by an equivalent
  // rigid object consisting of a point mass m2 at the CG and point
  // masses m2 at opposite ends, P and Q, of a line whose midpoint
  // is at the CG. The mass m2 is chosen so that the moment of inertia
  // I is 2m2*r^2, and m1 is chosen so that M = m1+2m2. When a force F
  // is applied at P, the axit of rotation will be orthogonal to the
  // plane containing the line PQ and the line of the force.
  ...
  // from P to the CG at a distance r.

  // The picture shortly after applying force F is as follows.
  ...
  //                 The mass     M = m1 + 2m2
  //    and the moment of inertia I = 2m2 * r^2
  ...
  //   PE = - F * (x + y)          (PE decreases as F moves forward)

  // The Lagrangian L = KE-PE is constant and is not affected by small
  // changes in x, y, x' or y'. The equations of motion are given by
  // the Euler-Langrange equations.
  
  // d/dt (d/dx' L) = d/dx L        (d/dx' and d/dx are partial derivatives)
  // d/dt (d/dy' L) = d/dy L        (d/dy' and d/dy are partial derivatives)
...
writef("applyforce: rft=%6.3f rfw=%6.3f rfl=%6.3f  before*n", rft, rfw, rfl)
  ...
  UNLESS pt=F0 DO rfw, rfl := rfw + pt*l, rfl - pt*w
  UNLESS pw=F0 DO rfl, rft := rfl + pw*t, rft - pw*l
  UNLESS pl=F0 DO rft, rfw := rft + pl*w, rfw - pl*t
...
//writef("Gravity force: ft=%13.3f fw=%13.3f fl=%13.3f*n", ft, fw, fl)
  ...
  // Drag effect
  ///ft := ft - massg*tdot*tdot / (8.0*95.0*95.0*steprate)
//writef("Drag force added => ft=%13.3f*n", ft)
  ...
  // Dihedral effect.
  rft := rft - 100.0 * wdot // In ft-poundals
//writef("Dihedral rotational force: wdot=%13.3f => rft=%13.3f*n", wdot, rft)
  ...
  // Now apply the controls: throttle, ailerons, elevator and rudder.
  
  // Throttle effect
  
  // First determine the engine target RPM.
  settargetrpm()

  // The rpm takes time to change.
  // On each call of step add a small proportion of the difference
  // between targetrpm and the current rpm. The -10.0 increases the
  // rate at which rpm reaches zero when the engine is turned off.
  
  TEST enginestarted
  THEN rpm := rpm + (targetrpm - rpm) / (4.0*steprate)
  ELSE rpm := rpm - 500.0 / steprate
  IF rpm < F0 DO rpm := F0
//writef("RPM=%n TargetRPM=%n*n", FIX rpm, FIX targetrpm)
...
writef(" throttle=%5.3f targetrpm= %n rpm=%n thrust=%n*n",
         throttle, FIX targetrpm, FIX rpm, FIX thrust)
writef(" aileron=%10.3f  elevator=%10.3f  rudder=%10.3f*n",
         aileron, elevator, rudder)
writef(" ft =%10.3f  fw=%10.3f  fl=%10.3f*n",  ft,  fw,  fl)
writef(" rft=%10.3f rfw=%10.3f rfl=%10.3f*n", rft, rfw, rfl)
//abort(1789)
  ...
  rtdot := rtdot + rft / mntt // Rotation rates in radians per second.
  rwdot := rwdot + rfw / mntw
  rldot := rldot + rfl / mntl

  // Apply rotation rate damping
  // The rate of rotation is approximately halved in one second
  // if no rotational force is applied.
  rtdot := rtdot - rtdot * 0.99 / steprate
  rwdot := rwdot - rwdot * 0.99 / steprate
  rldot := rldot - rldot * 0.99 / steprate


  rotatet(rtdot / steprate)
  rotatew(rwdot / steprate)
  rotatel(rldot / steprate)
  ...
  tdot := tdot + ft/massg/steprate
  wdot := wdot + fw/massg/steprate
  ldot := ldot + fl/massg/steprate
...
//writef("step: stepcount=%n steprate=%9.3f*n", stepcount, steprate)
...
writef("throttle=%5.3f tdot=%5.3f targetrpm=%6.3f*n", throttle, tdot, targetrpm)
  }
}
...
{ DEFAULT:
  ...
  CASE 1: // 'T'  Take off position
    writef("*ninitposition(1): T for take off position*n")
  ...
  CASE 2: // 'A'  Position on the glide slope -- level due north
    writef("*ninitposition(2): A Glide slope*n")
  ...
  CASE 3: // 'L'  Set flying level at 10000 ft at 65mph -- level due north
    writef("*ninitposition(3): L Level flight*n")
  ...
  CASE 4: // 'I'  Nearly inverted at 10000 ft to test stability.
    writef("*ninitposition(4): I Inverted*n")
    cgn,    cgw,    cgh    := -20000.0, F0, 10000.0    // Height of 10000 ft
    cgndot, cgwdot, cghdot :=     95.0, F0,    F0      // 65mph = 95 ft/s
    tdot,   wdot,   ldot   :=   cgndot, F0,    F0
  ...
  CASE 5: // 'W'  Level flight at 1 ft to test wheels.
    writef("*ninitposition(5): W Level flight at 2 ft*n")
  ...
  CASE 6: // 'V'  30 degree climb
    writef("*ninitposition(6): V 30 degree climb*n")
    cgn,    cgw,    cgh    := -20000.0, F0, 10000.0    // Height of 10000 ft
    cgndot, cgwdot, cghdot :=     95.0, F0,    F0      // 65mph = 95 ft/s
    tdot,   wdot,   ldot   :=   cgndot, F0,    F0
  ...
  CASE 7: // 'B'  Right wing 45 degrees down
    writef("*ninitposition(7): B Right wing 45 degrees dow*nn")
    cgn,    cgw,    cgh    := -20000.0, F0, 10000.0    // Height of 10000 ft
    cgndot, cgwdot, cghdot :=     95.0, F0,    F0      // 65mph = 95 ft/s
    tdot,   wdot,   ldot   :=   cgndot, F0,    F0
...
{ LET mx = screenxsize/2
  ...
  { 
    ...
    drawf(20, my,     "Throttle=%5.3f Elevator=%6.3f Aileron=%6.3f Rudder=%6.3f",
